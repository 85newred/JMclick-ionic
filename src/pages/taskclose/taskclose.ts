...
@Component({
  selector: 'page-taskclose',
  templateUrl: 'taskclose.html',
})
...
export class TaskClosePage{
 ...
 async loadData() {
    ...
    this.http
        .post(
          "https://jmclicks.com/api/gettaskclose?token=" + val.token,
          {
      ...
      this.http.get<{badge_count: any, notifications: any}>('https://jmclicks.com/api/notifications/getTask?token=' + val.token,{params:{type:"Close"}}).subscribe(result => {
        let t = this;
