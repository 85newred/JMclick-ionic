...
@Component({
  selector: 'page-leaveapprovedetail',
  templateUrl: 'leaveapprovedetail.html',
})
export class LeaveapprovedetailPage {
  ...
  loadData(){
    ...
    this.storage.get('token').then((val) => {
      ...
      this.http.post('https://jmclicks.com/api/notifications/updateleaveapproved?token='    + val.token, {TargetId: this.LeaveId}).subscribe(result => {
        console.log(result)
      })
    });
  }
