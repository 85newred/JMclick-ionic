...
@Component({
  selector: 'page-leaverejectdetail',
  templateUrl: 'leaverejectdetail.html',
})

export class LeaverejectdetailPage {
  ...
  loadData(){
    ...
    this.storage.get('token').then((val) => {
      ...
      this.http.post('https://jmclicks.com/api/notifications/updateleaverejected?token='    + val.token, {TargetId: this.LeaveId}).subscribe(result => {
        console.log(result)
      })
    });
  }
