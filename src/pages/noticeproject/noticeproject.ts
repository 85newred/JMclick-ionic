...
import { NoticeprojectdetailPage } from '../noticeprojectdetail/noticeprojectdetail';
...
@Component({
  selector: 'page-noticeproject',
  templateUrl: 'noticeproject.html',
})

export class NoticeprojectPage {
  ...
  noticedetailpage = NoticeprojectdetailPage;
  ...
  noticedetail(){
    this.navCtrl.push(NoticeprojectdetailPage)
  }
  ...
  loadData(){
    ...
    this.storage.get('token').then((val) => {
      data = this.http.get('https://jmclicks.com/api/getprojectnotice?token=' + val.token );
    ...
    this.storage.get('token').then((val) => {
      this.http.get<{badge_count: any, notifications: any}>('https://jmclicks.com/api/notifications/getprojectnoticebadge?token=' + val.token).subscribe(result => {
        let that = this
